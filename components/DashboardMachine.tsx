...
import { 
  ...
  TableProperties, LayoutGrid, TrendingUp, Car
} from 'lucide-react';
...
const DashboardMachine: React.FC = () => {
  ...
  const initiateOrchestration = async () => {
    ...
    try {
      ...
      if (analysis) {
        setSelectedStrategyIndex(analysis.aiRecommendation.recommendedStrategyIndex);
        setSelectedVendorIndex(analysis.aiRecommendation.recommendedVendorIndex);
      }
  ...
  const currentVendor = sourcingAnalysis?.vendorEstimates[selectedVendorIndex ?? 0];

  // Dynamic Impact Metrics
  const impactMetrics = useMemo(() => {
    if (!sourcingAnalysis) return null;
    
    const lossAvoided = sourcingAnalysis.financialImpact.doNothingLossINR;
    const costOfStrategy = currentVendor?.repairEstimateINR || sourcingAnalysis.financialImpact.mitigationCostINR;
    const netValueProtected = lossAvoided - costOfStrategy;
    const roi = (netValueProtected / costOfStrategy) * 100;
    
    // Convert Jobs/Hr to Units Protected
    const throughput = currentStrategy?.projectedLineThroughput || 0;
    const downtimeAvoidedHours = sourcingAnalysis.financialImpact.avoidedDowntimeHours || 8;
    const unitsProtected = Math.round(throughput * downtimeAvoidedHours);

    return { netValueProtected, roi, unitsProtected, downtimeAvoidedHours };
  }, [sourcingAnalysis, currentStrategy, currentVendor]);

  const handleBooking = (vendorName: string, index: number) => {
                ...
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col xl:flex-row justify-between gap-6 items-center">
                   <div className="flex items-center gap-5">
                      ...
                      <div>
                         ...
                         <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Status: {selectedMachine.status} • Line: {selectedMachine.lineId} • Capacity: {selectedMachine.capacityUnitsHr} Jobs/Hr</p>
                      </div>
                   </div>
                   <div className="flex items-center gap-3">
                ...
                {selectedMachine.serviceStage ? (
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <div className="bg-slate-900 text-white p-8 rounded-[3rem] shadow-2xl relative overflow-hidden flex flex-col border border-white/5">
                       ...
                       <div className="mb-6 p-5 bg-white/5 rounded-2xl border border-white/10 relative overflow-hidden group">
                          <div className="absolute top-0 right-0 p-4 text-blue-500/10 group-hover:text-blue-500/20 transition-all"><Sparkles size={60}/></div>
                          <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-2 flex items-center gap-2">AI Optimization Logic</p>
                          ...
                          {sourcingAnalysis.loadBalancingOptions.map((opt, i) => (
                             <div key={i} className="space-y-2">
                                ...
                                >
                                   ...
                                   <p className="text-[10px] text-white/60 leading-tight mb-3">{opt.description}</p>
                                   <div className="grid grid-cols-2 gap-4">
                                      <div className="flex items-center gap-2 text-[8px] font-black uppercase text-white/80"><Activity size={10} className="text-blue-300"/> Line OEE: {opt.projectedLineOEE}%</div>
                                      <div className="flex items-center gap-2 text-[8px] font-black uppercase text-white/80"><Target size={10} className="text-emerald-300"/> Fleet Output: {opt.projectedLineThroughput} Jobs/Hr</div>
                                   </div>
                                </button>
                                {expandedReasoning === i && (
                                   <div className="p-4 bg-white/10 border border-white/20 rounded-2xl animate-in slide-in-from-top-2 text-[10px] leading-relaxed text-slate-300 font-medium shadow-inner">
                                      <p className="text-blue-300 font-black uppercase mb-2 tracking-widest flex items-center gap-2"><Info size={10}/> Technical Allocation Details</p>
                                      {opt.reasoning}
                                   </div>
                                )}
                             </div>
                       ...
                       {currentStrategy && (
                          <div className="flex-1 space-y-4">
                             <div className="bg-black/20 p-5 rounded-[2rem] border border-white/5">
                                <p className="text-[9px] font-black text-blue-400 uppercase mb-4 tracking-widest flex items-center gap-2">
                                   <TableProperties size={12} /> Live Fleet Redistribution Table (Jobs/Hr)
                                </p>
                                <div className="space-y-3">
                                   <div className="grid grid-cols-4 gap-2 text-[8px] font-black text-white/40 uppercase tracking-widest pb-2 border-b border-white/5">
                                      <span>Machine</span>
                                      <span className="text-center">Base Load</span>
                                      <span className="text-center">Shifted</span>
                                      <span className="text-right">Total Output</span>
                                   </div>
                                   {currentStrategy.steps.map((step, i) => {
                                      const machineInfo = machines.find(m => m.id === step.targetMachineId);
                                      const baseLoad = machineInfo?.currentLoadUnitsHr || 0;
                                      const totalOutput = baseLoad + step.additionalLoadUnits;
                                      const isCritical = totalOutput >= (machineInfo?.capacityUnitsHr || 0) * 0.95;

                                      return (
                                        <div key={i} className="grid grid-cols-4 gap-2 items-center text-[10px] font-bold text-white/90">
                                           <div className="flex flex-col">
                                              <span className="uppercase">{step.targetMachineId}</span>
                                              <span className="text-[8px] text-white/40 truncate">{machineInfo?.name}</span>
                                           </div>
                                           <div className="text-center font-mono">{baseLoad}</div>
                                           <div className="text-center font-mono text-blue-400">+{step.additionalLoadUnits}</div>
                                           <div className="text-right">
                                              <div className="flex flex-col items-end">
                                                 <span className={`font-mono ${isCritical ? 'text-rose-400 animate-pulse' : 'text-emerald-400'}`}>{totalOutput} J/H</span>
                                                 <div className="w-12 h-1 bg-white/10 rounded-full mt-1 overflow-hidden">
                                                    <div className={`h-full ${isCritical ? 'bg-rose-500' : 'bg-emerald-500'}`} style={{ width: `${step.newUtilization}%` }} />
                                                 </div>
                                              </div>
                                           </div>
                                        </div>
                                ...
                                <div className="mt-6 pt-4 border-t border-white/10 flex justify-between items-center">
                                   <span className="text-[9px] font-black uppercase text-white/40">Total Redistributed Throughput</span>
                                   <span className="text-sm font-black text-emerald-400">{currentStrategy.projectedLineThroughput} Jobs/Hr</span>
                                </div>
                             </div>
                          </div>
                       )}
                    ...
                    <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col">
                       ...
                       <div className="flex-1 space-y-4 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                          {sourcingAnalysis.vendorEstimates.map((v, i) => (
                             <div 
                               ...
                               className={`p-6 rounded-[2rem] border transition-all relative group cursor-pointer ${selectedVendorIndex === i ? 'border-blue-500 ring-2 ring-blue-500/10 bg-white shadow-xl' : 'bg-slate-50 border-slate-100 opacity-80 hover:opacity-100'}`}
                             >
                                ...
                                <div className="flex items-center justify-between border-t border-slate-50 pt-4 mt-2">
                                   ...
                                   <button 
                                     onClick={(e) => { e.stopPropagation(); handleBooking(v.vendorName, i); }} 
                                     className={`px-8 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${selectedVendorIndex === i ? 'bg-blue-600 text-white shadow-lg hover:bg-blue-700' : 'bg-slate-900 text-white hover:bg-black'}`}
                                   >
                                     Book & Sync
                                   </button>
                                </div>
                       ...
                       {/* IMPACTFUL DECISION SUMMARY FOOTER */}
                       <div className="mt-8 bg-slate-900 rounded-[2.5rem] p-6 text-white relative overflow-hidden group shadow-2xl">
                          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform"><TrendingUp size={100} /></div>
                          <div className="relative z-10 space-y-6">
                             <div className="flex justify-between items-center border-b border-white/10 pb-4">
                                <div>
                                   <p className="text-[9px] font-black text-blue-400 uppercase tracking-[0.2em] mb-1">Strategic Net Benefit</p>
                                   <h5 className="text-2xl font-black text-white">₹{impactMetrics?.netValueProtected.toLocaleString() || '0'}</h5>
                                </div>
                                <div className="text-right">
                                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1">Strategy ROI</p>
                                   <span className="text-xl font-black text-emerald-400">+{impactMetrics?.roi.toFixed(1) || '0'}%</span>
                                </div>
                             </div>
                             
                             <div className="grid grid-cols-2 gap-6">
                                <div className="flex items-center gap-4">
                                   <div className="p-3 bg-white/5 rounded-2xl text-blue-400"><Car size={20} /></div>
                                   <div>
                                      <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Vehicle Protection</p>
                                      <p className="text-xs font-black text-white">{impactMetrics?.unitsProtected || '0'} Total Units</p>
                                   </div>
                                </div>
                                <div className="flex items-center gap-4">
                                   <div className="p-3 bg-white/5 rounded-2xl text-amber-400"><Clock size={20} /></div>
                                   <div>
                                      <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Production Recovery</p>
                                      <p className="text-xs font-black text-white">{impactMetrics?.downtimeAvoidedHours || '0'} Hours Saved</p>
                                   </div>
                                </div>
                             </div>
                          </div>
