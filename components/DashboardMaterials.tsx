...
const DashboardMaterials: React.FC = () => {
    ...
    const interval = setInterval(() => {
      setInitialSyncProgress(prev => {
        ...
        if (next >= 100) {
          ...
          setTimeout(() => setIsInitialSync(false), 600);
          return 100;
        }
        return next;
      });
  ...
  const extendedData = useMemo(() => {
    const safariRisks = [
      { ComponentCode: "SAF-901", ComponentName: "Panoramic Roof Frame", Module: "Body", Model: "Safari", Plant: "Pune", UoM: "EA", LeadTimeDays: 28, DailyUsageEst: 10, OnHandQty: 15, AllocatedQty: 8, SafetyStock: 60, QualityHoldQty: 0, StdUnitCostINR: 9200, InventoryValueINR: 138000, NextReplenishmentETA: "2026-03-12" },
      { ComponentCode: "SAF-902", ComponentName: "Harman Audio Processor", Module: "Electrical", Model: "Safari", Plant: "Pune", UoM: "EA", LeadTimeDays: 45, DailyUsageEst: 15, OnHandQty: 22, AllocatedQty: 12, SafetyStock: 100, QualityHoldQty: 2, StdUnitCostINR: 18500, InventoryValueINR: 407000, NextReplenishmentETA: "2026-03-25" },
      { ComponentCode: "SAF-903", ComponentName: "Oryx White Panel Set", Module: "Body", Model: "Safari", Plant: "Pune", UoM: "SET", LeadTimeDays: 14, DailyUsageEst: 20, OnHandQty: 45, AllocatedQty: 30, SafetyStock: 120, QualityHoldQty: 0, StdUnitCostINR: 24000, InventoryValueINR: 1080000, NextReplenishmentETA: "2026-03-05" },
    ];
    return [...safariRisks, ...CSV_DATA];
  }, []);
  ...
  if (isInitialSync) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-20 text-center space-y-8 h-full bg-slate-50">
        ...
        <div className="space-y-4 max-w-md">
          ...
          <p className="text-slate-500 text-sm font-medium">Fetching real-time material telemetry and pending POs...</p>
        </div>
        <div className="w-64 h-2 bg-slate-200 rounded-full overflow-hidden border border-slate-100 shadow-sm">
          <div className="h-full bg-blue-600 transition-all duration-300" style={{ width: `${initialSyncProgress}%` }} />
        </div>
      </div>
    );
  }
      ...
      {showSurgeModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-6xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[95vh]">
               ...
               {surgeSyncing ? (
                 <div className="flex flex-col items-center justify-center p-20 text-center space-y-8 h-full">
                   ...
                   <div className="space-y-2">
                     ...
                     <p className="text-slate-500 text-sm font-medium">Establishing secure handshake with multi-tier ERP modules...</p>
                   </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                       ...
                       <div className="space-y-8">
                                    ...
                                    return (
                                      <div key={idx} className={`p-4 rounded-2xl border transition-all flex flex-col relative overflow-hidden ${
                                        isBest 
                                         ...
                                         <button onClick={() => handleExecutePO(comp.code)} className={`mt-4 py-2 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${isBest ? 'bg-emerald-600 text-white' : 'bg-slate-900 text-white hover:bg-black'}`}>Execute PO</button>
                                      </div>
                                    );
                                  })}
                               </div>
                          ...
                          {surgeResult.riskComponents?.filter((c: any) => !executedPOs.has(c.code)).length === 0 && (
                            <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 bg-emerald-50 rounded-3xl border border-dashed border-emerald-200">
                               ...
                               <p className="text-xs text-emerald-700 font-medium">All critical surge deficit POs have been triggered.</p>
                            </div>
                          )}
                       </div>
                    </div>
             ...
             <div className="p-8 bg-white border-t border-slate-100 flex gap-4 shrink-0">
                <button onClick={() => setShowSurgeModal(false)} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all">
                  Exit Analysis Hub
                </button>
             </div>
          </div>
